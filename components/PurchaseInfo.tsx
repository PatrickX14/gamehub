...
export function PurchaseInfo({ data, accessToken }: UserOrderProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {contextHolder}
      <SectionCard title={`Order summary`} description={""} hideHeader>
        {data.length > 0 ? (
          data.map((order) => (
            <div key={order.id}>
              <div className="flex items-center gap-4 mb-4" key={order.id}>
                <Image
                ...
                {order.status === "AWAITING_PAYMENT" ? (
                  <div className="flex gap-2">
                    <Button
                      variant="solid"
                      onClick={() =>
                        router.replace(`/orderpayment/${order.id}`)
                      }
                    >
                      Procceed to payment
                    </Button>
          ...
          <Empty description={"There are no purchase history"} />
        )}
      </SectionCard>
    </div>
  );
